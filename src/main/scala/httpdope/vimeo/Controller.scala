...
class Controller[F[_]](client: VimeoClient[F], system: SystemCommands[F])
  ...
  private def findThumbs(
    ...
    find(request, f) { (agent, forwardedFor) =>
      client.getPictures(uid, exp, agent, forwardedFor)
    }
  }
  ...
  private def findDownloads(
    ...
    find(request, f) { (agent, forwardedFor) =>
      client.getDownloadLinks(uid, exp, agent, forwardedFor)
    }
  }
...
object Controller {
  def apply[F[_]](config: VimeoConfig, client: Client[F], system: SystemCommands[F])
    (implicit F: Concurrent[F], clock: Clock[F]): F[ControllerLike[F]] = {

    config.accessToken match {
      case None => F.pure(new EmptyController[F]())
      case Some(token) =>
        VimeoClient[F](token, client).map { client =>
